...
export class AdminService {
  ...
  private fallbackUsers: AdminUserDTO[] = [
    ...
    },
    {
      ...
      profileDescription: 'Digital Content Creator',
      profileImageUrl: undefined
    },
    {
      id: 3,
      ...
      profileDescription: 'Software Developer',
      profileImageUrl: undefined
    },
    {
      id: 4,
      ...
      profileDescription: 'Premium Customer',
      profileImageUrl: undefined
    },
    {
      id: 5,
      ...
      profileDescription: 'Regular Customer',
      profileImageUrl: undefined
    }
  ];
  ...
  createUser(user: CreateAdminUserDTO): Observable<any> {
    if (this.useFallbackData) {
  ...
  activateUser(id: number): Observable<any> {
    if (this.useFallbackData) {
      ...
      if (user) {
        user.isActive = true;
        return of({ success: true, message: `User ${id} activated.` });
      } else {
        return throwError(() => new Error('User not found'));
      }
    }
    ...
    return this.http.post(`${this.baseUrl}/${id}/activate`, {}, { responseType: 'text' }).pipe(
      map(response => {
        console.log('User activated successfully');
        return { success: true, message: response || `User ${id} activated.` };
      }),
      catchError(this.handleError)
    );
  }
  ...
  deactivateUser(id: number): Observable<any> {
    if (this.useFallbackData) {
      ...
      if (user) {
        user.isActive = false;
        return of({ success: true, message: `User ${id} deactivated.` });
      } else {
        return throwError(() => new Error('User not found'));
      }
    }
    ...
    return this.http.post(`${this.baseUrl}/${id}/deactivate`, {}, { responseType: 'text' }).pipe(
      map(response => {
        console.log('User deactivated successfully');
        return { success: true, message: response || `User ${id} deactivated.` };
      }),
      catchError(this.handleError)
    );
  }
